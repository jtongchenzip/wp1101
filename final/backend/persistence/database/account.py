...
# async def edit(account_id: int, username: str, real_name: str) -> None:
...
#         fr" WHERE id = %(account_id)s",
#     )
#     params = param_maker(account_id=account_id, username=username, real_name=real_name)
#     sql, params = pyformat2psql(sql, params)
#     id_, = await pool_handler.pool.fetchrow(sql, *params)
#     return
...
async def delete(account_id: int) -> None:
    ...
    sql, params = pyformat2psql(sql, params)
    try:
        _ = await pool_handler.pool.fetchrow(sql, *params)
    except TypeError:
        raise exc.NotFound
    return
