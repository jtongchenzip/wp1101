...
export default (boardSize, mineNum) => {
    ...
    for(let x = 0; x < boardSize; x++) {
        for(let y = 0; y < boardSize; y++) {
            if(board[x][y].value == '💣') {
                // console.log("bomb");                
                if((x - 1 >= 0) && (board[x - 1][y].value !=='💣')) { board[x - 1][y].value++ }
                if((x + 1 < boardSize) && (board[x + 1][y].value !=='💣')) { board[x + 1][y].value++ }
                if((y - 1 >= 0) && (board[x][y - 1].value !=='💣')) { board[x][y - 1].value++ }
                if((y + 1 < boardSize) && (board[x][y + 1].value !=='💣')) { board[x][y + 1].value++ }
                if((x - 1 >= 0) && (y - 1 >= 0) && (board[x - 1][y - 1].value !=='💣')) { board[x - 1][y - 1].value++ }
                if((x + 1 < boardSize) && (y - 1 >= 0) && (board[x + 1][y - 1].value !=='💣')) { board[x + 1][y - 1].value++ }
                if((x - 1 >= 0) && (y + 1 < boardSize) && (board[x - 1][y + 1].value !=='💣')) { board[x - 1][y + 1].value++ }
                if((x + 1 < boardSize) && (y + 1 < boardSize) && (board[x + 1][y + 1].value !=='💣')) { board[x + 1][y + 1].value++ }
                // console.log(x, y);
                // printBoard()
            }
        }
